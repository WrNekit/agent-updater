...
UPDATE_URL = "https://raw.githubusercontent.com/WrNekit/agent-updater/refs/heads/main/7.py"
...
def normalize_code(data):
    ...
    try:
        ...
        # Приводим переводы строк к единому виду
        text = text.replace('\r\n', '\n')
...
def code_has_changed(new_code):
    ...
    и new_code. Возвращает кортеж (изменился ли, текущий хэш, новый хэш).
    """
...
def delayed_restart(delay, new_file):
    """
    Ждёт delay секунд, затем запускает новый процесс с new_file и завершает текущий.
    """
    time.sleep(delay)
    try:
...
def perform_update_sync(update_url):
    ...
    записывает новый код во временный файл, создаёт резервную копию и возвращает результат.
    """
...
def update_endpoint():
    ...
    2. Если есть, синхронно выполняем обновление.
    3. Если обновление успешно, запускаем новый процесс через 2 секунды.
    """
...
if __name__ == '__main__':
    ...
    # Рекомендуется для production использовать gunicorn или другой WSGI-сервер.
