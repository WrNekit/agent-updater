...
# URL обновления (должен указывать на raw-версию нового бинарного файла, скомпилированного в exe)
UPDATE_URL = "https://raw.githubusercontent.com/WrNekit/agent-updater/refs/heads/main/agent_new.exe"
...
def is_compiled():
    """
    Определяет, что агент запущен в виде бинарника (exe).
    Обычно sys.argv[0] заканчивается на .exe при использовании PyInstaller.
    """
...
def normalize_code(data):
    ...
    Нормализует содержимое кода:
      - Декодирует UTF-8,
      - Заменяет CRLF на LF,
      - Применяет .strip() для удаления начальных/конечных пробелов,
...
def code_has_changed(new_code):
    """
    Сравнивает нормализованное содержимое текущего исполняемого файла и new_code.
    Возвращает (изменился ли, current_hash, new_hash).
    """
...
def check_for_updates():
    """
    Скачивает новый бинарный файл с UPDATE_URL, нормализует его содержимое и сравнивает с текущим.
    Если код изменился (логически), возвращает информацию об обновлении.
    """
    try:
...
def delayed_restart_exe(delay, new_file):
    """
    Для бинарника (.exe): ждёт delay секунд, затем записывает временный batch‑файл,
    который через небольшую задержку заменяет текущий exe новым и запускает его.
    После запуска batch‑файл сам удаляет себя.
    """
    time.sleep(delay)
    current_exe = os.path.abspath(sys.argv[0])
    bat_file = os.path.join(os.path.dirname(current_exe), "update.bat")
    # Формируем содержимое batch-файла:
    # timeout ждет 3 секунды (можно настроить), затем копирует new_file поверх текущего и запускает его.
...
def delayed_restart(delay, new_file):
    """
    Если агент запущен как скрипт (не бинарник), заменяет текущий процесс с помощью os.execv.
    """
    time.sleep(delay)
...
def perform_update_sync(update_url):
    """
    Синхронно скачивает новый бинарный файл, сравнивает его с текущим,
    сохраняет новый файл с отметкой времени, удаляет старые файлы обновлений,
    делает резервную копию текущего файла и возвращает результат.
    """
    ...
    try:
        ...
        # Для бинарника используем расширение .exe, для скрипта — .pyw (на Windows) или .py (на Linux)
        new_file_ext = ".exe" if is_compiled() else (".pyw" if is_windows() else ".py")
...
def is_compiled():
    """
    Определяет, что агент запущен как бинарник, проверяя расширение sys.argv[0].
    """
    return sys.argv[0].lower().endswith(".exe")
...
def update_endpoint():
    ...
    Принудительный вызов обновления:
      ...
      2. Если обновление найдено, выполняем скачивание и замену с созданием нового файла с отметкой времени.
      3. Если агент запущен как бинарник (.exe), используем batch-скрипт для замены (delayed_restart_exe);
         иначе – os.execv для скриптов.
    """
