...
UPDATE_URL = "https://raw.githubusercontent.com/WrNekit/agent-updater/refs/heads/main/24.py"
VERISONAPP = '1.0.1'
...
def code_has_changed(new_code):
    ...
    try:
        ...
        # Файл уже не существует (например, переименован в бэкап).
        print("[INFO] Текущий файл агента не найден — считаем, что код изменился.")
        return True, None, None

    except Exception as e:
        # Любые другие ошибки (проблемы с доступом, чтением и т.д.)
...
def check_for_updates():
    try:
        ...
        if changed:
            # Если текущий файл не найден или не прочитан (current_hash = None), 
            # code_has_changed() вернёт (True, None, None). Логируем понятное сообщение:
            if current_hash is None:
                print("[INFO] Старый файл агента недоступен (или отсутствует). Считаем, что код изменился.")
