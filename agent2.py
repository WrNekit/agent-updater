...
UPDATE_URL = "https://raw.githubusercontent.com/WrNekit/agent-updater/refs/heads/main/agent2.py"
...
def code_has_changed(new_data, is_py=False):
    ...
    try:
        ...
        if not os.path.exists(current_file):
            logging.error(f"[ERROR] Текущий файл {current_file} не существует!")
        ...
        # Логируем хэши для отладки
        logging.info(f"Old hash: {old_hash}")
        logging.info(f"New hash: {new_hash}")
        logging.info(f"Hashes changed: {old_hash != new_hash}")

        return old_hash != new_hash, old_hash, new_hash
    except Exception as e:
        logging.error(f"[ERROR] Ошибка при вычислении хэша: {e}")
        return True, None, None
...
def check_for_updates():
    ...
    try:
        ...
        logging.info(f"Old hash: {old_hash}")
        logging.info(f"New hash: {new_hash}")
        logging.info(f"Hashes changed: {changed}")
        ...
        logging.error(f"check_for_updates exception: {e}")
