...
def shutdown_server():
    """Вызывает метод завершения работы сервера Werkzeug."""
    func = request.environ.get('werkzeug.server.shutdown')
    if func is None:
        print("[ERROR] Не найден метод завершения сервера.")
        raise RuntimeError('Not running with the Werkzeug Server')
    func()
    print("[INFO] Сервер Flask завершает работу.")
...
def perform_update_sync(update_url):
    ...
    try:
        ...
        # Выбираем расширение: для бинарника (.exe) или для скрипта (.pyw или .py)
        if is_compiled():
            new_file_ext = ".exe"
        else:
