...
UPDATE_URL = "https://raw.githubusercontent.com/WrNekit/agent-updater/refs/heads/main/16.py"
VERISONAPP = '1.0.1'
...
def perform_update_sync(update_url):
    ...
    try:
        ...
        # Перемещаем новый файл на место текущего (если требуется, либо оставляем с отметкой времени)
        # Если нужно, чтобы имя обновлённого файла было оригинальным, можно переименовать:
        # shutil.move(new_file_name, current_file)
        # В данном варианте новый файл оставляем под именем с отметкой времени.
        
        print("[INFO] Обновление завершено. Запускаем новый процесс Flask...")
        
        # Запускаем новый процесс с новым файлом (если вы хотите запускать именно обновлённый файл с отметкой времени)
        try:
        ...
        # Ждем несколько секунд, чтобы убедиться, что новый процесс работает
        time.sleep(5)
        flask_running = False
