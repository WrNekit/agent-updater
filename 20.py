...
def perform_update_sync(update_url):
    ...
    try:
        ...
        print("[INFO] Заменяем текущий файл и создаем резервную копию...")
        # Создаем резервную копию текущего файла и заменяем его новым
        ...
        # Запускаем новый процесс, "отсоединив" его от текущего
        try:
            new_process = subprocess.Popen(
                [sys.executable, current_file],
                creationflags=subprocess.CREATE_NEW_PROCESS_GROUP,  # отсоединяем процесс
                close_fds=True
            )
        ...
        # Ждем несколько секунд, чтобы убедиться, что новый процесс запустился
        time.sleep(5)
        flask_running = False
        ...
        if not flask_running:
            print("[WARNING] Новый процесс Flask не запущен, пробуем повторный запуск...")
            try:
                new_process = subprocess.Popen(
                    [sys.executable, current_file],
